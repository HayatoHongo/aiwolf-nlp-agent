...
def parse_finish_packet(log_path):
    ...
    for line in reversed(lines):
        if "Request.FINISH" in line:
            ...
            # 解析status_map
            status_dict = {}
            for m in re.finditer(r"'([^']+)': <Status\.(ALIVE|DEAD): 'ALIVE'|'DEAD'>", status_map_str):
                name, status = m.group(1), m.group(2)
                status_dict[name] = status
            # 解析role_map
            role_dict = {}
            for m in re.finditer(r"'([^']+)': <Role\.([A-Z]+): '[A-Z]+'?>", role_map_str):
...
def judge_winner(status_dict, role_dict):
    ...
    villager_alive = sum(1 for name, status in status_dict.items() if status == "ALIVE" and role_dict[name] in villager_roles)
    werewolf_alive = sum(1 for name, status in status_dict.items() if status == "ALIVE" and role_dict[name] == "WEREWOLF")
    if werewolf_alive == 0:
        return "VILLAGER"
